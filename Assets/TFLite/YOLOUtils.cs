...
public class YOLOUtils
{
    ...
    // It is the output from a yolov8n object detection model.
    ...
    // These are the outputs from yolov8n-seg.
    public static List<Box> Segmentation(float[,] output0, float[,,] output1, float scoreThreshold)
    {
        ...
        o0 = ops.StridedSlice(o0, new int[]{84,0,0,0}, new int[]{output0.GetLength(0),1,1,output0.GetLength(1)}, new int[]{1,1,1,1}); // 32, 1, 1, 2100
        ...
        Tensor o1 = new Tensor(flattened.GetLength(0), flattened.GetLength(1), flattened); // 6400, 1, 1, 32

        Tensor product = ops.MatMul(o0, true, o1, true); // 2100, 1, 1, 6400
        o0.Dispose();
        o1.Dispose();
